...
class WhisperMemoryTracker:
    def __init__(self, model_name: str, save_path: str):
        ...
        self.memory_measurements = deque(maxlen=50)  # Store last 50 measurements
    ...
    def print_summary(self):
        ...
        print(f"\nCPU Usage:")
        print(f"  Initial CPU: {summary['cpu']['initial_percent']:.1f}%")
        print(f"  Peak CPU: {summary['cpu']['peak_percent']:.1f}%")
        print(f"  Initial RAM: {summary['cpu']['initial_ram_gb']:.2f} GB")
        print(f"  Current RAM: {summary['cpu']['current_ram_gb']:.2f} GB")
        
        if 'gpu' in summary:
            print(f"\nGPU Usage:")
            print(f"  Initial Allocated: {summary['gpu']['initial_allocated_gb']:.2f} GB")
            print(f"  Peak Allocated: {summary['gpu']['peak_allocated_gb']:.2f} GB")
            print(f"  Average Allocated: {summary['gpu']['average_allocated_gb']:.2f} GB")
            print(f"  Current Allocated: {summary['gpu']['current_allocated_gb']:.2f} GB")
            print(f"  Current Cached: {summary['gpu']['current_cached_gb']:.2f} GB")
...
def transcribe_batch(batch, model, processor, memory_tracker, split, batch_idx):
    with torch.no_grad():
        ...
        # Convert features to FP16 if model is in FP16
...
def evaluate_model(model, processor, dataset, metrics, memory_tracker, split, batch_size=2):
    # Create partial function with memory tracker
    map_fn = partial(
        transcribe_batch,
        model=model,
        processor=processor,
    ...
    def process_batch(batch):
        ...
        result = map_fn(batch, batch_idx=batch_counter)
        batch_counter += 1
        return result
    ...
    # Add this line to capture end time
    end = time.time()
    ...
    # In your evaluate_model function, add this before returning
    average_cpu_usage = sum(
        [m['cpu_percent'] for m in memory_tracker.memory_measurements]) / len(
            memory_tracker.memory_measurements) if memory_tracker.memory_measurements else 0
...
def main():
    ...
    model_configs = {
        ...
        "bnb_fp4_32": {
            "quantization": "bnb_fp4_32"
        },
        "bnb_fp4_16": {
            "quantization": "bnb_fp4_16"
        },
        "bnb_nf4_32": {
            "quantization": "bnb_nf4_32"
        },
        "bnb_nf4_16": {
            "quantization": "bnb_nf4_16"
        },
        "bnb_fp4_16_double": {
            "quantization": "bnb_fp4_16_double"
        },
        "bnb_nf4_16_double": {
            "quantization": "bnb_nf4_16_double"
        }
    }
    ...
    for run_name, run_results in results.items():
        ...
        if "avg_cpu_percent" in run_results['metrics']:
            print(f"  Avg CPU: {run_results['metrics']['avg_cpu_percent']:.2f}%")
            print(f"  Peak CPU: {run_results['metrics']['peak_cpu_percent']:.2f}%")
