...
def main():
    ...
    # EVALUATE PYTORCH DYNAMICALLY QUNATIZED MODEL
    print("Evaluating PyTorch model:")
    pytorch_ptdq_clean_scores, pytorch_ptdq_clean_transcriptions = evaluate_model(
        quantized_model, processor, processed_test_data_clean, metrics, args.batch_size
    )
    pytorch_ptdq_other_scores, pytorch_ptdq_other_transcriptions = evaluate_model(
        quantized_model, processor, processed_test_data_other, metrics, args.batch_size
    )
    quantized_model_size = get_model_disk_size_in_mb(quantized_model)
    print(f"PyTorch quantized model size: {quantized_model_size:.2f} MB")

    # Save metrics and transcriptions
    pytorch_ptdq_clean_metrics = {
        "metrics": pytorch_ptdq_clean_scores,
        "model_size_mb": quantized_model_size,
        "model_type": "PyTorch PTDQ",
        "model_name": args.model,
    }

    # Save metrics and transcriptions
    pytorch_ptdq_other_metrics = {
        "metrics": pytorch_ptdq_other_scores,
        "model_size_mb": quantized_model_size,
        "model_type": "qint4",
        "model_name": args.model,
    }

    with open(os.path.join(args.save_path, "pytorch_ptdq_clean_metrics.json"), "w") as f:
        json.dump(pytorch_ptdq_clean_metrics, f, indent=2)

    with open(os.path.join(args.save_path, "pytorch_ptdq_clean_transcriptions.json"), "w") as f:
        json.dump(pytorch_ptdq_clean_transcriptions, f, indent=2)

    with open(os.path.join(args.save_path, "pytorch_ptdq_other_metrics.json"), "w") as f:
        json.dump(pytorch_ptdq_other_metrics, f, indent=2)

    with open(os.path.join(args.save_path, "pytorch_ptdq_other_transcriptions.json"), "w") as f:
        json.dump(pytorch_ptdq_other_transcriptions, f, indent=2)
