...
def main():
    ...
    qmodel_int2 = WhisperForConditionalGeneration.from_pretrained(args.model).to(device)
    qmodel_int2.config.forced_decoder_ids = None

    quantize(qmodel_int2, weights=qint2)
    ...
    # EVALUATE QINT2
    print("Evaluating qint2 model:")
    qint2_clean_scores, qint2_clean_transcriptions = evaluate_model(
        qmodel_int2, processor, processed_test_data_clean, metrics, args.batch_size
    )
    qint2_other_scores, qint2_other_transcriptions = evaluate_model(
        qmodel_int2, processor, processed_test_data_other, metrics, args.batch_size
    )
    qint2_model_size = get_model_disk_size_in_mb(qmodel_int2)
    print(f"qint2 model size: {qint2_model_size:.2f} MB")

    # Save metrics and transcriptions
    qint2_clean_metrics = {
        "metrics": qint2_clean_scores,
        "model_size_mb": qint2_model_size,
        "model_type": "qint2",
    ...
    # Save metrics and transcriptions
    qint2_other_metrics = {
        "metrics": qint2_other_scores,
        "model_size_mb": qint2_model_size,
        "model_type": "qint2",
        "model_name": args.model,
    }

    with open(os.path.join(args.save_path, "qint2_clean_metrics.json"), "w") as f:
        json.dump(qint2_clean_metrics, f, indent=2)

    with open(os.path.join(args.save_path, "qint2_clean_transcriptions.json"), "w") as f:
        json.dump(qint2_clean_transcriptions, f, indent=2)

    with open(os.path.join(args.save_path, "qint2_other_metrics.json"), "w") as f:
        json.dump(qint2_other_metrics, f, indent=2)

    with open(os.path.join(args.save_path, "qint2_other_transcriptions.json"), "w") as f:
        json.dump(qint2_other_transcriptions, f, indent=2)
