...
from transformers import BitsAndBytesConfig, HqqConfig, WhisperForConditionalGeneration
...
def apply_static_quantization(
    ...
    type_mapping = {"int4": qint4, "int8": qint8, "float8": qfloat8}

    if weight_type not in type_mapping or activation_type not in type_mapping:
        raise ValueError(
            f"Unsupported quantization types: weight_type={weight_type}, activation_type={activation_type}"
        )
...
def save_model_metrics(
    model: torch.nn.Module, model_name: str, metrics: Dict[str, Any], save_path: str
) -> None:
    """
