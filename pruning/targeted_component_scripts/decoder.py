...
L1_PRUNING_DIR = os.path.join(RESULTS_DIR, "l1_decoder_pruning")  # Changed to indicate decoder-only
...
def apply_l1_pruning(
    model, amount=0.3, target_modules=None, make_permanent=False, decoder_only=False
):
    """
    ...
    Args:
        ...
        decoder_only: Whether to only prune decoder modules (ignoring encoder)
    ...
    for name, module in model.named_modules():
        ...
        if any(isinstance(module, m) for m in target_modules):
            total_modules += 1
            # Only include decoder modules if decoder_only is True
            if not decoder_only or "decoder" in name:
    ...
    if decoder_only:
        print(
            f"Note: Only pruning decoder modules ({decoder_modules} decoder modules out of {total_modules} total modules)"
        )
...
def load_whisper_model(
    model_name, device, pruning_amount=None, make_permanent=True, decoder_only=False
):
    """
    ...
    Args:
        ...
        make_permanent: Whether to make pruning permanent
        decoder_only: Whether to only prune the decoder (ignoring encoder)
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            ...
            if decoder_only:
                print("Note: Only pruning decoder modules")
...
def create_plots(results, metric_names, plot_dir, decoder_only=True):
    """
    ...
    Args:
        ...
        decoder_only: Whether this is decoder-only pruning (True) or whole model pruning (False)
    """
    print("\nGenerating plots...")
    ...
    for model_name, model_results in results.items():
        if "baseline" in model_name:
            ...
            try:
                if decoder_only:
                    # For decoder-only models (format: "l1_decoder_XX_clean" or "l1_decoder_XX_other")
    ...
    pruning_type = "Decoder-Only" if decoder_only else "Whole Model"
    ...
    for percent in pruning_percentages:
        ...
        if percent == 0:
            ...
            if decoder_only:
                model_name = f"l1_decoder_{percent}_clean"
                if model_name not in results:
                    model_name = f"l1_decoder_{percent}_other"
            else:
...
def main():
    ...
    # Define the pruning percentages to test (0% to 50% in 10% increments for decoder-only)
    ...
    for percent in pruning_percentages:
        if percent > 0:  # Skip 0% as it's already in baseline
            model_configs[f"l1_decoder_{percent}"] = {
                "pruning_amount": percent / 100,  # Convert percentage to fraction
                "decoder_only": True,  # Only prune decoder
            }
                ...
                try:
                    ...
                    if isinstance(scores, dict) and "error" not in scores:
                        ...
                        results[f"{model_name}_{split}"] = {
                            ...
                            "decoder_only": config.get(
                                "decoder_only", False
                            ),  # Add decoder_only flag
    ...
    create_plots(results=results, metric_names=safe_metrics, plot_dir=PLOTS_DIR, decoder_only=True)

    # Print summary
    print("\n" + "=" * 60)
    print("L1 UNSTRUCTURED DECODER-ONLY PRUNING EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    print("\nResults for different decoder pruning percentages:")
    for percent in pruning_percentages:
        if percent == 0:
            continue

        model_key = f"l1_decoder_{percent}_clean"
    ...
    print(
        "\nNote: This experiment focused on pruning only the decoder component of the Whisper model"
    )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("WHISPER DECODER-ONLY L1 UNSTRUCTURED PRUNING EXPERIMENT")
    print("=" * 80)
    print(
        "This experiment applies L1 unstructured pruning ONLY to the decoder part of the Whisper model."
    )
