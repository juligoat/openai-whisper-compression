...
EMBEDDING_PRUNING_DIR = os.path.join(RESULTS_DIR, "embedding_pruning")
PLOTS_DIR = os.path.join(EMBEDDING_PRUNING_DIR, "plots")
MODELS_DIR = os.path.join(EMBEDDING_PRUNING_DIR, "models")

for directory in [RESULTS_DIR, EMBEDDING_PRUNING_DIR, PLOTS_DIR, MODELS_DIR]:
...
def calculate_pruned_dense_size(model, pruning_threshold=0.0):
    ...
    for name, param in model.named_parameters():
        ...
        # We're only interested in embedding layers for this analysis
...
def save_sparse_model(model, output_path):
    ...
    for name, param in model.state_dict().items():
        ...
        if "embed" in name.lower():
            param_type = "embedding_" + param_type
...
def apply_token_embedding_pruning(model, amount=0.3, make_permanent=False):
    """
    Apply L1 unstructured pruning specifically to token embedding layers in a Whisper model.
    ...
    for name, module in model.named_modules():
        # Look for embedding layers in both encoder and decoder
    ...
    if not params_to_prune:
        print("Warning: No token embedding parameters found to prune!")
        return model

    print(
        f"Found {len(params_to_prune)} token embedding modules to prune with L1 unstructured pruning, amount={amount}"
    )
    ...
    try:
        ...
        print(f"Error during token embedding pruning: {e}")

    print(
        f"Successfully applied pruning to {success_count}/{len(params_to_prune)} token embedding modules"
    )
...
def calculate_sparsity(model):
    ...
    focusing on token embedding layers.
    ...
    Returns:
        tuple: (
            ...
            total_parameters,
            non_zero_parameters,
            embedding_sparsity_percentage,
            embedding_total_parameters,
            embedding_non_zero_parameters
        )
    """
    total_params = 0
    zero_params = 0
    embedding_total_params = 0
    embedding_zero_params = 0
    ...
    for name, param in model.named_parameters():
        ...
        if is_embedding:
            embedding_total_params += param.numel()
            embedding_zero_params += torch.sum(param == 0).item()
    ...
    # Calculate embedding-specific sparsity
    if embedding_total_params == 0:
        embedding_sparsity = 0.0
    else:
        embedding_sparsity = 100.0 * embedding_zero_params / embedding_total_params

    overall_non_zero = total_params - zero_params
    embedding_non_zero = embedding_total_params - embedding_zero_params
...
def load_whisper_model(model_name, device, pruning_amount=None, make_permanent=True):
    """
    Load Whisper model and optionally apply token embedding pruning.
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            print(f"Applying token embedding pruning with amount={pruning_amount}")
            model = apply_token_embedding_pruning(
            ...
            (
                ...
                embedding_sparsity,
                embedding_total_params,
                embedding_non_zero_params,
            ) = calculate_sparsity(model)

            print(f"Overall model sparsity: {overall_sparsity:.2f}%")
            print(f"Total parameters: {total_params:,}")
            print(f"Non-zero parameters: {non_zero_params:,}")
            print(f"Token embedding sparsity: {embedding_sparsity:.2f}%")
            print(f"Token embedding parameters: {embedding_total_params:,}")
            print(f"Token embedding non-zero parameters: {embedding_non_zero_params:,}")
...
def create_plots(results, metric_names, plot_dir):
    ...
    for model_name, model_results in results.items():
        if "baseline" in model_name:
            ...
            # Extract percentage from model name (e.g., "embedding_10")
            try:
    ...
    for metric in metric_names:
        ...
        plt.xlabel("Token Embedding Pruning Percentage (%)")
        plt.ylabel(metric)
        plt.title(f"{metric} vs Token Embedding Pruning Percentage")
        plt.grid(True)
    ...
    for percent in pruning_percentages:
        ...
        model_name = f"embedding_{percent}_clean" if percent > 0 else "baseline_clean"
        if model_name not in results and percent > 0:
            model_name = f"embedding_{percent}_other"
        if model_name not in results and percent == 0:
    ...
    plt.title("Model Size vs Token Embedding Pruning Percentage")
    plt.grid(True)
    plt.legend()
    ...
    # Create embedding sparsity vs. overall sparsity plot
    plt.figure(figsize=(10, 6))
    ...
    for percent in pruning_percentages:
        ...
        if model_name in results:
            if "embedding_sparsity" in results[model_name]:
                embedding_sparsity.append((percent, results[model_name]["embedding_sparsity"]))
            if "overall_sparsity" in results[model_name]:
                overall_sparsity.append((percent, results[model_name]["overall_sparsity"]))

    # Sort by pruning percentage
    embedding_sparsity.sort(key=lambda x: x[0])
    overall_sparsity.sort(key=lambda x: x[0])

    # Plot sparsity
    if embedding_sparsity:
        plt.plot(
            [p for p, _ in embedding_sparsity],
            [s for _, s in embedding_sparsity],
            marker="o",
    ...
    plt.xlabel("Token Embedding Pruning Percentage (%)")
    plt.ylabel("Sparsity (%)")
    plt.title("Sparsity vs Token Embedding Pruning Percentage")
    plt.grid(True)
    plt.legend()

    # Save the plot
    plot_path = os.path.join(plot_dir, "sparsity_vs_pruning.png")
    plt.savefig(plot_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"Saved plot to {plot_path}")
    ...
    for percent in pruning_percentages:
        ...
        if model_name in results:
            ...
            if "embedding_total_parameters" in results[model_name]:
                embedding_total_params.append(
                    (percent, results[model_name]["embedding_total_parameters"])
                )
            if "embedding_non_zero_parameters" in results[model_name]:
                embedding_non_zero_params.append(
                    (percent, results[model_name]["embedding_non_zero_parameters"])
                )

    # Sort by pruning percentage
    total_params.sort(key=lambda x: x[0])
    non_zero_params.sort(key=lambda x: x[0])
    embedding_total_params.sort(key=lambda x: x[0])
    embedding_non_zero_params.sort(key=lambda x: x[0])
    ...
    if embedding_total_params:
        plt.plot(
            [p for p, _ in embedding_total_params],
            [t / 1_000_000 for _, t in embedding_total_params],
            marker="^",
            linestyle="--",
            label="Embedding total parameters",
        )
    if embedding_non_zero_params:
        plt.plot(
            [p for p, _ in embedding_non_zero_params],
            [nz / 1_000_000 for _, nz in embedding_non_zero_params],
            marker="x",
            linestyle="--",
            label="Embedding non-zero parameters",
        )

    plt.xlabel("Token Embedding Pruning Percentage (%)")
    plt.ylabel("Parameters (millions)")
    plt.title("Parameter Count vs Token Embedding Pruning Percentage")
    plt.grid(True)
    plt.legend()
...
def main():
    ...
    save_path = EMBEDDING_PRUNING_DIR
    ...
    for percent in pruning_percentages:
        if percent > 0:  # Skip 0% as it's already in baseline
            model_configs[f"embedding_{percent}"] = {
                ...
                try:
                    ...
                    if isinstance(scores, dict) and "error" not in scores:
                        ...
                        results[f"{model_name}_{split}"] = {
                            ...
                            "embedding_total_parameters": embedding_total_params,
                            "embedding_non_zero_parameters": embedding_non_zero_params,
    ...
    all_results_path = os.path.join(EMBEDDING_PRUNING_DIR, "all_results.json")
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("TOKEN EMBEDDING PRUNING EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    if "baseline_clean" in results:
        ...
        if "embedding_total_parameters" in baseline:
            print(f"  Token Embedding Parameters: {baseline['embedding_total_parameters']:,}")
    ...
    for percent in pruning_percentages:
        ...
        model_key = f"embedding_{percent}_clean"
        if model_key in results:
            ...
            if "baseline_clean" in results:
                ...
                if (
                    "embedding_non_zero_parameters" in result
                    and "embedding_non_zero_parameters" in baseline
                ):
                    embedding_param_change = f"{(result['embedding_non_zero_parameters'] - baseline['embedding_non_zero_parameters']) / baseline['embedding_non_zero_parameters'] * 100:+.2f}%"

            print(f"\n{percent}% token embedding pruning:")
            ...
            if "embedding_sparsity" in result:
                print(f"  Token Embedding Sparsity: {result['embedding_sparsity']:.2f}%")
            ...
            if "embedding_total_parameters" in result:
                print(f"  Token Embedding Parameters: {result['embedding_total_parameters']:,}")
            if "embedding_non_zero_parameters" in result:
                print(
                    f"  Token Embedding Non-zero Parameters: {result['embedding_non_zero_parameters']:,} ({embedding_param_change})"
                )

    print("\nPlots saved to:", PLOTS_DIR)
    print("Sparse models saved to:", MODELS_DIR)
    print("Detailed metrics saved to:", EMBEDDING_PRUNING_DIR)
