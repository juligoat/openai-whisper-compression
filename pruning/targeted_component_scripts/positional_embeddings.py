...
POSITIONAL_PRUNING_DIR = os.path.join(RESULTS_DIR, "positional_pruning")
PLOTS_DIR = os.path.join(POSITIONAL_PRUNING_DIR, "plots")
MODELS_DIR = os.path.join(POSITIONAL_PRUNING_DIR, "models")

for directory in [RESULTS_DIR, POSITIONAL_PRUNING_DIR, PLOTS_DIR, MODELS_DIR]:
...
def calculate_pruned_dense_size(model, pruning_threshold=0.0):
    ...
    for name, param in model.named_parameters():
        ...
        # We're only interested in positional embedding layers for this analysis
        is_positional_embedding = any(
            pos_name in name.lower() for pos_name in ["position", "pos_embed", "positional"]
        )

        if is_positional_embedding and param.dim() > 1:  # Only consider embedding matrices
            # Find pruned weights
            ...
            # For non-positional embedding parameters, size remains the same
...
def save_sparse_model(model, output_path):
    ...
    for name, param in model.state_dict().items():
        ...
        # Mark positional embeddings for special tracking
        if any(pos_name in name.lower() for pos_name in ["position", "pos_embed", "positional"]):
...
def apply_positional_embedding_pruning(model, amount=0.3, make_permanent=False):
    """
    Apply L1 unstructured pruning specifically to positional embedding layers in a Whisper model.
    ...
    Returns:
        Pruned model
    """
    # Find positional embedding layers by name pattern
    params_to_prune = []
    positional_modules = {}

    # First, identify all positional embedding modules and their parameters
    for name, module in model.named_modules():
        # Look for positional embedding layers in both encoder and decoder
        if any(pos_name in name.lower() for pos_name in ["position", "pos_embed", "positional"]):
            if hasattr(module, "weight"):
                params_to_prune.append((module, "weight"))
                positional_modules[name] = module
                print(f"Found positional embedding layer: {name}")

    if not params_to_prune:
        print("Warning: No positional embedding parameters found to prune!")
        return model

    print(
        f"Found {len(params_to_prune)} positional embedding modules to prune with L1 unstructured pruning, amount={amount}"
    )
    ...
    try:
        ...
        print(f"Error during positional embedding pruning: {e}")

    print(
        f"Successfully applied pruning to {success_count}/{len(params_to_prune)} positional embedding modules"
    )
...
def calculate_sparsity(model):
    ...
    focusing on positional embedding layers.
...
def load_whisper_model(model_name, device, pruning_amount=None, make_permanent=True):
    """
    Load Whisper model and optionally apply positional embedding pruning.
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            print(f"Applying positional embedding pruning with amount={pruning_amount}")
            model = apply_positional_embedding_pruning(
            ...
            print(f"Positional embedding non-zero parameters: {positional_non_zero_params:,}")
...
def create_plots(results, metric_names, plot_dir):
    ...
    for model_name, model_results in results.items():
        if "baseline" in model_name:
            ...
            # Extract percentage from model name (e.g., "positional_10")
            try:
    ...
    for metric in metric_names:
        ...
        plt.xlabel("Positional Embedding Pruning Percentage (%)")
        plt.ylabel(metric)
        plt.title(f"{metric} vs Positional Embedding Pruning Percentage")
        plt.grid(True)
    ...
    for percent in pruning_percentages:
        ...
        model_name = f"positional_{percent}_clean" if percent > 0 else "baseline_clean"
        if model_name not in results and percent > 0:
            model_name = f"positional_{percent}_other"
        if model_name not in results and percent == 0:
    ...
    plt.title("Model Size vs Positional Embedding Pruning Percentage")
    plt.grid(True)
    plt.legend()
    ...
    # Create positional embedding sparsity vs. overall sparsity plot
    plt.figure(figsize=(10, 6))
    ...
    plt.xlabel("Positional Embedding Pruning Percentage (%)")
    plt.ylabel("Sparsity (%)")
    plt.title("Sparsity vs Positional Embedding Pruning Percentage")
    plt.grid(True)
    plt.legend()
    ...
    if positional_total_params:
        plt.plot(
            ...
            marker="^",
            linestyle="--",
            label="Positional total parameters",
        )
    if positional_non_zero_params:
        plt.plot(
            ...
            marker="x",
            linestyle="--",
            label="Positional non-zero parameters",
        )

    plt.xlabel("Positional Embedding Pruning Percentage (%)")
    plt.ylabel("Parameters (millions)")
    plt.title("Parameter Count vs Positional Embedding Pruning Percentage")
    plt.grid(True)
    plt.legend()
...
def main():
    ...
    save_path = POSITIONAL_PRUNING_DIR
    ...
    for percent in pruning_percentages:
        if percent > 0:  # Skip 0% as it's already in baseline
            model_configs[f"positional_{percent}"] = {
                ...
                try:
                    ...
                    if isinstance(scores, dict) and "error" not in scores:
                        ...
                        results[f"{model_name}_{split}"] = {
                            ...
                            "positional_non_zero_parameters": positional_non_zero_params,
    ...
    all_results_path = os.path.join(POSITIONAL_PRUNING_DIR, "all_results.json")
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("POSITIONAL EMBEDDING PRUNING EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    for percent in pruning_percentages:
        ...
        model_key = f"positional_{percent}_clean"
        if model_key in results:
            ...
            if "baseline_clean" in results:
                ...
                if (
                    "positional_non_zero_parameters" in result
                    and "positional_non_zero_parameters" in baseline
                ):
                    positional_param_change = f"{(result['positional_non_zero_parameters'] - baseline['positional_non_zero_parameters']) / baseline['positional_non_zero_parameters'] * 100:+.2f}%"

            print(f"\n{percent}% positional embedding pruning:")
            ...
            if "positional_non_zero_parameters" in result:
                print(
                    f"  Positional Embedding Non-zero Parameters: {result['positional_non_zero_parameters']:,} ({positional_param_change})"
                )

    print("\nPlots saved to:", PLOTS_DIR)
    print("Sparse models saved to:", MODELS_DIR)
    print("Detailed metrics saved to:", POSITIONAL_PRUNING_DIR)
