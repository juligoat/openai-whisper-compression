...
L1_PRUNING_DIR = os.path.join(
    RESULTS_DIR, "l1_decoder_ffn_pruning"
)  # Changed to indicate decoder FFN-only
...
def calculate_model_gflops(model):
    ...
    flops_by_type = {"encoder": 0, "decoder": 0, "decoder_ffn": 0, "other": 0}
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            ...
            if "encoder" in name:
                ...
                # Check if this is an FFN component in the decoder
                if is_ffn_layer(name):
                    flops_by_type["decoder_ffn"] += non_zero_ops
                else:
    ...
    total_flops = (
        ...
        + avg_sequence_length * (flops_by_type["decoder"] + flops_by_type["decoder_ffn"])
        + flops_by_type["other"]
    )
...
def is_ffn_layer(name):
    """
    Determine if a layer is part of the FFN (Feed-Forward Network) in the decoder.

    In Transformer-based models like Whisper, the FFN usually consists of two linear
    layers with an activation function in between. These are typically found in each
    decoder layer after the attention mechanisms.

    Args:
        name: The name of the layer/parameter

    Returns:
        bool: True if this is an FFN layer in the decoder, False otherwise
    """
    # First check if it's in the decoder
    if "decoder" not in name:
        return False

    # In Whisper, FFN components in the decoder typically include these terms
    ffn_indicators = [
        # Common names for FFN layers in transformer models
        "fc1",
        "fc2",  # Feed-forward components
        "feed_forward",
        "ffn",  # Direct FFN naming
        "linear1",
        "linear2",  # Linear layer naming
        "mlp",  # MLP naming convention
        "dense_1",
        "dense_2",
        "w1",
        "w2",
        "wo",  # Other common names
        # Handle Whisper-specific naming conventions for FFN layers
        "decoder.layers",
        "feed_forward",  # Specific to Whisper architecture
    ]

    # In whisper, the FFN is often in the decoder's feed-forward component
    # Check for multiple indicators to increase confidence
    if any(indicator in name for indicator in ffn_indicators):
        # Ensure it's not an attention component
        ...
        if not any(attn_term in name for attn_term in attention_indicators):
            return True
...
def calculate_sparsity(model, component=None):
    ...
    Args:
        ...
        component: Optional component to focus on ('encoder', 'decoder', 'decoder_ffn', or None for whole model)
    ...
    for name, param in model.named_parameters():
        ...
        if (
            ...
            or component == "decoder_ffn"
            and not (is_ffn_layer(name) and "decoder" in name)
        ):
            continue
...
def apply_l1_pruning(
    model, amount=0.3, target_modules=None, make_permanent=False, decoder_ffn_only=False
):
    """
    ...
    Args:
        ...
        decoder_ffn_only: Whether to only prune FFN components in the decoder
    ...
    for name, module in model.named_modules():
        ...
        if any(isinstance(module, m) for m in target_modules):
            ...
            # Only include decoder FFN modules if decoder_ffn_only is True
            if decoder_ffn_only:
                if "decoder" in name and is_ffn_layer(name):
                    params_to_prune.append((module, "weight"))
                    ffn_modules += 1

                    # Debug information to understand which layers are being targeted
                    if hasattr(module, "weight"):
                        print(f"Selected FFN layer: {name}, Shape: {module.weight.shape}")
            else:
                # Regular pruning approach if not decoder_ffn_only
                params_to_prune.append((module, "weight"))
                if "decoder" in name:
                    decoder_modules += 1
    ...
    if decoder_ffn_only:
        print(
            f"Note: Only pruning decoder FFN modules ({ffn_modules} FFN modules out of {total_modules} total modules)"
        )
...
def load_whisper_model(
    model_name, device, pruning_amount=None, make_permanent=True, decoder_ffn_only=False
):
    """
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            ...
            if decoder_ffn_only:
                print("Note: Only pruning FFN components in the decoder")
            model = apply_l1_pruning(
                model,
                amount=pruning_amount,
                make_permanent=make_permanent,
                decoder_ffn_only=decoder_ffn_only,
            )
            ...
            ffn_sparsity, ffn_total, ffn_nonzero = calculate_sparsity(model, "decoder_ffn")
            ...
            print(f"Decoder FFN sparsity: {ffn_sparsity:.2f}%")
            print(f"Total parameters: {total_params:,}")
            print(f"Non-zero parameters: {non_zero_params:,}")
            print(f"FFN parameters: {ffn_total:,} ({ffn_total/total_params*100:.2f}% of model)")
...
def create_plots(results, metric_names, plot_dir, decoder_ffn_only=True):
    """
    ...
    Args:
        ...
        decoder_ffn_only: Whether this is decoder FFN-only pruning (True) or whole model pruning (False)
    """
    print("\nGenerating plots...")
    ...
    for model_name, model_results in results.items():
        if "baseline" in model_name:
            ...
            try:
                if decoder_ffn_only:
                    # For decoder_ffn-only models (format: "l1_decoder_ffn_XX_clean" or "l1_decoder_ffn_XX_other")
    ...
    pruning_type = "Decoder FFN-Only" if decoder_ffn_only else "Whole Model"
    ...
    for percent in pruning_percentages:
        ...
        if percent == 0:
            ...
            if decoder_ffn_only:
                model_name = f"l1_decoder_ffn_{percent}_clean"
                if model_name not in results:
                    model_name = f"l1_decoder_ffn_{percent}_other"
            else:
...
def main():
    ...
    # Define the pruning percentages to test (0% to 50% in 10% increments for decoder-FFN-only)
    ...
    for percent in pruning_percentages:
        if percent > 0:  # Skip 0% as it's already in baseline
            model_configs[f"l1_decoder_ffn_{percent}"] = {
                "pruning_amount": percent / 100,  # Convert percentage to fraction
                "decoder_ffn_only": True,  # Only prune FFN components in the decoder
            }
    ...
    for model_name, config in model_configs.items():
        ...
        try:
            ...
            model = load_whisper_model(
                ...
                make_permanent=True,
                decoder_ffn_only=config.get("decoder_ffn_only", False),
            )
            ...
            print(f"Non-zero parameters: {non_zero_params:,}")
            print(
                f"Decoder FFN parameters: {ffn_total:,} ({ffn_total/total_params*100:.2f}% of model)"
            )
                ...
                try:
                    ...
                    if isinstance(scores, dict) and "error" not in scores:
                        ...
                        results[f"{model_name}_{split}"] = {
                            ...
                            "decoder_ffn_only": config.get(
                                "decoder_ffn_only", False
                            ),  # Add decoder_ffn_only flag
                            ...
                            "ffn_parameters": ffn_total,  # Add FFN parameter count
    ...
    create_plots(
        results=results, metric_names=safe_metrics, plot_dir=PLOTS_DIR, decoder_ffn_only=True
    )

    # Print summary
    print("\n" + "=" * 60)
    print("L1 UNSTRUCTURED DECODER FFN-ONLY PRUNING EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    print("\nResults for different decoder FFN pruning percentages:")
    for percent in pruning_percentages:
        if percent == 0:
            continue

        model_key = f"l1_decoder_ffn_{percent}_clean"
        if model_key in results:
            ...
            if "ffn_sparsity" in result:
                print(f"  Decoder FFN Sparsity: {result['ffn_sparsity']:.2f}%")
            ...
            if "ffn_parameters" in result:
                print(
                    f"  Decoder FFN Parameters: {result['ffn_parameters']:,} ({result['ffn_parameters']/result['total_parameters']*100:.2f}% of model)"
                )
    ...
    print(
        "\nNote: This experiment focused on pruning only the FFN components in the decoder of the Whisper model"
    )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("WHISPER DECODER FFN-ONLY L1 UNSTRUCTURED PRUNING EXPERIMENT")
    print("=" * 80)
    print("This experiment applies L1 unstructured pruning ONLY to the Feed-Forward Network (FFN)")
    print("components in the decoder part of the Whisper model.")
