...
L1_PRUNING_DIR = os.path.join(
    RESULTS_DIR, "l1_encoder_ffn_pruning"
)  # Changed to indicate encoder FFN-only
...
def calculate_model_gflops(model):
    ...
    flops_by_type = {"encoder": 0, "encoder_ffn": 0, "decoder": 0, "other": 0}
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            ...
            if "encoder" in name:
                # Check if this is an FFN component in the encoder
                if is_ffn_layer(name):
                    flops_by_type["encoder_ffn"] += non_zero_ops
                else:
...
def save_sparse_model(model, output_path):
    ...
    for name, param in model.state_dict().items():
        ...
        if "encoder" in name:
            if is_ffn_layer(name):
                param_type = "encoder_ffn"
            else:
                param_type = "encoder_other"
        elif "decoder" in name:
            param_type = "decoder"
...
def is_ffn_layer(name):
    """
    Determine if a layer is part of the FFN (Feed-Forward Network).

    In Transformer-based models like Whisper, the FFN usually consists of two linear
    layers with an activation function in between.

    Args:
        name: The name of the layer/parameter

    Returns:
        bool: True if this is an FFN layer, False otherwise
    """
    # Determine if we're looking at an encoder or decoder component
    ...
    # If we're only interested in encoder FFN components, filter out decoder modules
    if is_decoder:
        return False

    # In Whisper, FFN components typically include these terms
    ffn_indicators = [
    ...
    # In transformer models, the FFN is often in the feed-forward component
...
def calculate_sparsity(model, component=None):
    ...
    Args:
        ...
        component: Optional component to focus on ('encoder', 'encoder_ffn', 'decoder', or None for whole model)
    ...
    for name, param in model.named_parameters():
        ...
        if (
            ...
            or component == "encoder_ffn"
            and not (is_ffn_layer(name) and "encoder" in name)
        ):
            continue
...
def apply_l1_pruning(
    model, amount=0.3, target_modules=None, make_permanent=False, encoder_ffn_only=False
):
    """
    ...
    Args:
        ...
        encoder_ffn_only: Whether to only prune FFN components in the encoder
    ...
    for name, module in model.named_modules():
        ...
        if any(isinstance(module, m) for m in target_modules):
            ...
            # Only include encoder FFN modules if encoder_ffn_only is True
            if encoder_ffn_only:
                if "encoder" in name and is_ffn_layer(name):
                ...
                # Regular pruning approach if not encoder_ffn_only
                params_to_prune.append((module, "weight"))
                if "encoder" in name:
                    encoder_modules += 1
    ...
    if encoder_ffn_only:
        print(
            f"Note: Only pruning encoder FFN modules ({ffn_modules} FFN modules out of {total_modules} total modules)"
        )
...
def load_whisper_model(
    model_name, device, pruning_amount=None, make_permanent=True, encoder_ffn_only=False
):
    """
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            ...
            if encoder_ffn_only:
                print("Note: Only pruning FFN components in the encoder")
            model = apply_l1_pruning(
                model,
                amount=pruning_amount,
                make_permanent=make_permanent,
                encoder_ffn_only=encoder_ffn_only,
            )
            ...
            ffn_sparsity, ffn_total, ffn_nonzero = calculate_sparsity(model, "encoder_ffn")
            ...
            print(f"Encoder FFN sparsity: {ffn_sparsity:.2f}%")
            print(f"Total parameters: {total_params:,}")
            print(f"Non-zero parameters: {non_zero_params:,}")
            print(
                f"Encoder FFN parameters: {ffn_total:,} ({ffn_total/total_params*100:.2f}% of model)"
            )
...
def create_plots(results, metric_names, plot_dir, encoder_ffn_only=True):
    """
    ...
    Args:
        ...
        encoder_ffn_only: Whether this is encoder FFN-only pruning (True) or whole model pruning (False)
    """
    print("\nGenerating plots...")
    ...
    for model_name, model_results in results.items():
        if "baseline" in model_name:
            ...
            try:
                if encoder_ffn_only:
                    # For encoder_ffn-only models (format: "l1_encoder_ffn_XX_clean" or "l1_encoder_ffn_XX_other")
    ...
    pruning_type = "Encoder FFN-Only" if encoder_ffn_only else "Whole Model"
    ...
    for percent in pruning_percentages:
        ...
        if percent == 0:
            ...
            if encoder_ffn_only:
                model_name = f"l1_encoder_ffn_{percent}_clean"
                if model_name not in results:
                    model_name = f"l1_encoder_ffn_{percent}_other"
            else:
...
def main():
    ...
    # Define the pruning percentages to test (0% to 50% in 10% increments for encoder-FFN-only)
    ...
    for percent in pruning_percentages:
        if percent > 0:  # Skip 0% as it's already in baseline
            model_configs[f"l1_encoder_ffn_{percent}"] = {
                "pruning_amount": percent / 100,  # Convert percentage to fraction
                "encoder_ffn_only": True,  # Only prune FFN components in the encoder
            }
    ...
    for model_name, config in model_configs.items():
        ...
        try:
            ...
            model = load_whisper_model(
                ...
                make_permanent=True,
                encoder_ffn_only=config.get("encoder_ffn_only", False),
            )
                ...
                try:
                    ...
                    if isinstance(scores, dict) and "error" not in scores:
                        ...
                        results[f"{model_name}_{split}"] = {
                            ...
                            "encoder_ffn_only": config.get(
                                "encoder_ffn_only", False
                            ),  # Add encoder_ffn_only flag
    ...
    create_plots(
        results=results, metric_names=safe_metrics, plot_dir=PLOTS_DIR, encoder_ffn_only=True
    )

    # Print summary
    print("\n" + "=" * 60)
    print("L1 UNSTRUCTURED ENCODER FFN-ONLY PRUNING EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    print("\nResults for different encoder FFN pruning percentages:")
    for percent in pruning_percentages:
        if percent == 0:
            continue

        model_key = f"l1_encoder_ffn_{percent}_clean"
        if model_key in results:
            ...
            if "ffn_sparsity" in result:
                print(f"  Encoder FFN Sparsity: {result['ffn_sparsity']:.2f}%")
            ...
            if "ffn_parameters" in result:
                print(
                    f"  Encoder FFN Parameters: {result['ffn_parameters']:,} ({result['ffn_parameters']/result['total_parameters']*100:.2f}% of model)"
                )
    ...
    print(
        "\nNote: This experiment focused on pruning only the FFN components in the encoder of the Whisper model"
    )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("WHISPER ENCODER FFN-ONLY L1 UNSTRUCTURED PRUNING EXPERIMENT")
    print("=" * 80)
    print("This experiment applies L1 unstructured pruning ONLY to the Feed-Forward Network (FFN)")
    print("components in the encoder part of the Whisper model.")
