...
L1_PRUNING_DIR = os.path.join(
    RESULTS_DIR, "l1_layer_norm_pruning"
)  # Changed to indicate layer norm pruning
...
def calculate_model_gflops(model):
    ...
    flops_by_type = {"encoder": 0, "decoder": 0, "layer_norm": 0, "other": 0}
    ...
    for name, module in model.named_modules():
        ...
        # For LayerNorm, estimate FLOPs
        elif isinstance(module, torch.nn.LayerNorm):
            if hasattr(module, "weight") and hasattr(module, "bias"):
                # Simplified estimate: 2 ops per element (scale and shift)
                # This is much lower than matrix multiplies, but we'll track it separately
                layer_size = module.weight.numel()
                flops_by_type["layer_norm"] += 2 * layer_size
                ...
                if hasattr(module.weight, "detach"):
                    ...
                    non_zero_params += (module.bias != 0).sum().item()
                else:
                    non_zero_params += module.weight.numel() + module.bias.numel()
...
def save_sparse_model(model, output_path):
    ...
    for name, param in model.state_dict().items():
        ...
        if "layer_norm" in name or "layernorm" in name or "ln" in name:
...
def is_layer_norm_parameter(name):
    """
    Determine if a parameter is part of a layer normalization component.

    Args:
        name: The name of the parameter

    Returns:
        bool: True if this is a layer norm parameter, False otherwise
    """
    # Check for common layer normalization naming patterns
    ...
    # Check if any layer norm indicator is in the name
    if any(indicator in name.lower() for indicator in layer_norm_indicators):
        return True
...
def calculate_sparsity(model, component=None):
    ...
    Args:
        ...
        component: Optional component to focus on ('encoder', 'decoder', 'layer_norm', or None for whole model)
    ...
    for name, param in model.named_parameters():
        ...
        if (
            ...
            or component == "layer_norm"
            and not is_layer_norm_parameter(name)
        ):
            continue
...
def apply_l1_pruning(
    model, amount=0.3, target_modules=None, make_permanent=False, layer_norm_only=False
):
    """
    ...
    Args:
        ...
        layer_norm_only: Whether to only prune layer normalization parameters

    Returns:
        Pruned model
    """
    # For layer norm pruning, we specifically target LayerNorm modules
    if layer_norm_only:
        target_modules = [torch.nn.LayerNorm]
    ...
    for name, module in model.named_modules():
        ...
        if any(isinstance(module, m) for m in target_modules):
            ...
            # Only include layer norm modules if layer_norm_only is True
            if layer_norm_only:
                # For layer norm, we want to prune both weights and biases
                if isinstance(module, torch.nn.LayerNorm):
                    if hasattr(module, "weight"):
                        params_to_prune.append((module, "weight"))
                        layer_norm_modules += 1
                        print(f"Selected layer norm weight: {name}, Shape: {module.weight.shape}")

                    if hasattr(module, "bias"):
                        params_to_prune.append((module, "bias"))
                        print(f"Selected layer norm bias: {name}, Shape: {module.bias.shape}")
            else:
                # Regular pruning approach if not layer_norm_only
                params_to_prune.append((module, "weight"))
    ...
    print(
        f"Found {len(params_to_prune)} parameters to prune with L1 unstructured pruning, amount={amount}"
    )

    if layer_norm_only:
        print(
            f"Note: Only pruning layer normalization modules ({layer_norm_modules} layer norm modules out of {total_modules} total modules)"
        )
    ...
    print(f"Successfully applied pruning to {success_count}/{len(params_to_prune)} parameters")
    ...
    if make_permanent:
        ...
        for module, param_name in params_to_prune:
            try:
                # Only make permanent if the module has a mask attribute
                if hasattr(module, f"{param_name}_mask"):
                    prune.remove(module, param_name)
                    permanent_count += 1
            except Exception as e:
                print(f"Could not make pruning permanent for {module}.{param_name}: {e}")
        print(f"Made pruning permanent for {permanent_count}/{len(params_to_prune)} parameters")

    return model


def load_whisper_model(
    model_name, device, pruning_amount=None, make_permanent=True, layer_norm_only=False
):
    """
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            ...
            if layer_norm_only:
                print("Note: Only pruning layer normalization parameters")
            model = apply_l1_pruning(
                model,
                amount=pruning_amount,
                make_permanent=make_permanent,
                layer_norm_only=layer_norm_only,
            )
            ...
            layer_norm_sparsity, layer_norm_total, layer_norm_nonzero = calculate_sparsity(
                model, "layer_norm"
            )
            ...
            print(f"Layer normalization sparsity: {layer_norm_sparsity:.2f}%")
            print(f"Total parameters: {total_params:,}")
            print(f"Non-zero parameters: {non_zero_params:,}")
            print(
                f"Layer norm parameters: {layer_norm_total:,} ({layer_norm_total/total_params*100:.2f}% of model)"
            )
...
def create_plots(results, metric_names, plot_dir, layer_norm_only=True):
    """
    ...
    Args:
        ...
        layer_norm_only: Whether this is layer-norm-only pruning (True) or whole model pruning (False)
    """
    print("\nGenerating plots...")
    ...
    for model_name, model_results in results.items():
        if "baseline" in model_name:
            ...
            try:
                if layer_norm_only:
                    # For layer_norm-only models (format: "l1_layer_norm_XX_clean" or "l1_layer_norm_XX_other")
    ...
    pruning_type = "Layer Normalization Only" if layer_norm_only else "Whole Model"
    ...
    for percent in pruning_percentages:
        ...
        if percent == 0:
            ...
            if layer_norm_only:
                model_name = f"l1_layer_norm_{percent}_clean"
                if model_name not in results:
                    model_name = f"l1_layer_norm_{percent}_other"
            else:
...
def main():
    ...
    # Define the pruning percentages to test (0% to 50% in 10% increments for layer-norm-only)
    ...
    for percent in pruning_percentages:
        if percent > 0:  # Skip 0% as it's already in baseline
            model_configs[f"l1_layer_norm_{percent}"] = {
                "pruning_amount": percent / 100,  # Convert percentage to fraction
                "layer_norm_only": True,  # Only prune layer normalization parameters
            }
    ...
    for model_name, config in model_configs.items():
        ...
        try:
            ...
            model = load_whisper_model(
                ...
                make_permanent=True,
                layer_norm_only=config.get("layer_norm_only", False),
            )
    ...
    create_plots(
        results=results, metric_names=safe_metrics, plot_dir=PLOTS_DIR, layer_norm_only=True
    )

    # Print summary
    print("\n" + "=" * 60)
    print("L1 UNSTRUCTURED LAYER NORMALIZATION ONLY PRUNING EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    print("\nResults for different layer normalization pruning percentages:")
    for percent in pruning_percentages:
        if percent == 0:
            continue

        model_key = f"l1_layer_norm_{percent}_clean"
        if model_key in results:
            ...
            print(f"\n{percent}% layer normalization pruning:")
            ...
            if "layer_norm_sparsity" in result:
                print(f"  Layer Normalization Sparsity: {result['layer_norm_sparsity']:.2f}%")
            ...
            if "layer_norm_parameters" in result:
                print(
                    f"  Layer Normalization Parameters: {result['layer_norm_parameters']:,} ({result['layer_norm_parameters']/result['total_parameters']*100:.2f}% of model)"
                )
    ...
    print(
        "\nNote: This experiment focused on pruning only the layer normalization parameters of the Whisper model"
    )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("WHISPER LAYER NORMALIZATION ONLY L1 UNSTRUCTURED PRUNING EXPERIMENT")
    print("=" * 80)
    print(
        "This experiment applies L1 unstructured pruning ONLY to the layer normalization parameters"
    )
    print("in the Whisper model (both encoder and decoder).")
