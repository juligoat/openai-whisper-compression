...
def is_cross_attention_layer(name):
    ...
    In Transformer-based models like Whisper, the cross-attention connects the decoder
    to the encoder outputs. This is different from self-attention where the decoder
    attends to its own previous outputs.

    Args:
        name: The name of the layer/parameter

    Returns:
        bool: True if this is a cross-attention layer in the decoder, False otherwise
    """
    ...
    # In transformer models, cross-attention components typically include these terms
    cross_attention_indicators = [
        "encoder_attn",
    ...
    # Check if any cross-attention indicator is in the name
    if any(indicator in name.lower() for indicator in cross_attention_indicators):
        return True

    # Sometimes cross-attention is simply the second attention module in decoder layers
    # This is model-specific and may need adjustment based on Whisper's architecture
    if "attn" in name.lower() and "layer" in name.lower():
        # In many transformer implementations, the second attention block is cross-attention
        # This is a heuristic and may need refinement for Whisper specifically
        layer_parts = name.split(".")
        for i, part in enumerate(layer_parts):
            if "attn" in part.lower() and i > 0 and "1" in layer_parts[i - 1]:
                return True
    ...
    for name, module in model.named_modules():
        ...
        if any(isinstance(module, m) for m in target_modules):
            ...
            if decoder_cross_attention_only:
                ...
                full_name = f"{name}"
                if "decoder" in full_name and is_cross_attention_layer(full_name):
