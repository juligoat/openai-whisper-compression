...
L1_PRUNING_DIR = os.path.join(RESULTS_DIR, "l1_output_proj_pruning")
...
def apply_l1_pruning_to_output_projection(model, amount=0.3, make_permanent=False):
    """
    Apply L1 unstructured pruning ONLY to the output projection layer (proj_out) of a Whisper model.
    ...
    Returns:
        Pruned model
    """
    # Find the output projection layer (proj_out)
    proj_out_layer = None
    for name, module in model.named_modules():
        if name == "proj_out":
            proj_out_layer = module
            break

    if proj_out_layer is None:
        print("Warning: Could not find proj_out layer! No pruning applied.")
        return model

    print(f"Found output projection layer (proj_out) with shape {proj_out_layer.weight.shape}")
    print(f"Applying L1 unstructured pruning with amount={amount}")

    # Apply L1 unstructured pruning to the proj_out layer only
    try:
        prune.l1_unstructured(proj_out_layer, name="weight", amount=amount)

        # Calculate sparsity
        if hasattr(proj_out_layer, "weight_mask"):
            pruned_weights = (proj_out_layer.weight_mask == 0).sum().item()
            total_weights = proj_out_layer.weight_mask.numel()
            sparsity = 100.0 * pruned_weights / total_weights
            print(
                f"Applied pruning to proj_out: {sparsity:.2f}% sparse ({pruned_weights}/{total_weights} weights)"
            )

        # Make pruning permanent if requested
        if make_permanent and hasattr(proj_out_layer, "weight_mask"):
            prune.remove(proj_out_layer, "weight")
            print("Made pruning permanent")

    except Exception as e:
        print(f"Error applying pruning to proj_out: {e}")
...
def load_whisper_model(model_name, device, pruning_amount=None, make_permanent=True):
    """
    Load Whisper model and optionally apply pruning to output projection only.

    Args:
        model_name: The Whisper model name
        device: Device to load the model to
        pruning_amount: Amount to prune (0.0 to 0.5) or None for no pruning
        make_permanent: Whether to make pruning permanent
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            print(
                f"Applying L1 unstructured pruning to output projection with amount={pruning_amount}"
            )
            model = apply_l1_pruning_to_output_projection(
...
def create_plots(results, metric_names, plot_dir):
    ...
    for metric in metric_names:
        ...
        plt.ylabel(metric)
        plt.title(f"{metric} vs L1 Unstructured Pruning (Output Projection Only)")
        plt.grid(True)
    ...
    plt.title("Model Size vs L1 Unstructured Pruning (Output Projection Only)")
    plt.grid(True)
    plt.legend()
    ...
    plt.ylabel("GFLOPs")
    plt.title(
        "Computational Complexity (GFLOPs) vs L1 Unstructured Pruning (Output Projection Only)"
    )
    plt.grid(True)
    plt.legend()
    ...
    plt.title("Parameter Count vs L1 Unstructured Pruning (Output Projection Only)")
    plt.grid(True)
    plt.legend()
...
def main():
    ...
    # Define the pruning percentages to test (0% to 50%)
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("L1 UNSTRUCTURED PRUNING (OUTPUT PROJECTION ONLY) EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    for percent in pruning_percentages:
        ...
        if model_key in results:
            ...
            print(f"\n{percent}% pruning (output projection only):")
