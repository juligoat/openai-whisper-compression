...
L1_PRUNING_DIR = os.path.join(
    RESULTS_DIR, "l1_encoder_attn_pruning"
)  # Changed to indicate encoder attention-only
...
def calculate_model_gflops(model):
    ...
    flops_by_type = {"encoder": 0, "encoder_attn": 0, "decoder": 0, "other": 0}
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            ...
            if "encoder" in name:
                # Check if this is an attention component in the encoder
                if is_attention_layer(name):
                    flops_by_type["encoder_attn"] += non_zero_ops
                else:
...
def is_attention_layer(name):
    """
    Determine if a layer is part of the self-attention mechanism in the encoder.
    ...
    Returns:
        bool: True if this is an attention layer in the encoder, False otherwise
    """
    # Determine if we're looking at an encoder component
...
def calculate_sparsity(model, component=None):
    ...
    Args:
        ...
        component: Optional component to focus on ('encoder', 'encoder_attn', 'decoder', or None for whole model)
    ...
    for name, param in model.named_parameters():
        ...
        if (
            ...
            or component == "encoder_attn"
            and not (is_attention_layer(name) and "encoder" in name)
        ):
            continue
...
def apply_l1_pruning(
    model, amount=0.3, target_modules=None, make_permanent=False, encoder_attention_only=False
):
    """
    ...
    Args:
        ...
        encoder_attention_only: Whether to only prune attention components in the encoder
    ...
    for name, module in model.named_modules():
        ...
        if any(isinstance(module, m) for m in target_modules):
            ...
            # Only include encoder attention modules if encoder_attention_only is True
            if encoder_attention_only:
                # Use full module name path to check if it's an attention layer
                full_name = f"{name}"
                if "encoder" in full_name and is_attention_layer(full_name):
                ...
                # Regular pruning approach if not encoder_attention_only
                params_to_prune.append((module, "weight"))
                if "encoder" in name:
                    encoder_modules += 1
    ...
    if encoder_attention_only:
        print(
            f"Note: Only pruning encoder attention modules ({attn_modules} attention modules out of {total_modules} total modules)"
        )
...
def load_whisper_model(
    model_name, device, pruning_amount=None, make_permanent=True, encoder_attention_only=False
):
    """
    ...
    try:
        ...
        if pruning_amount is not None and pruning_amount > 0:
            ...
            if encoder_attention_only:
                print("Note: Only pruning self-attention components in the encoder")
            model = apply_l1_pruning(
                model,
                amount=pruning_amount,
                make_permanent=make_permanent,
                encoder_attention_only=encoder_attention_only,
            )
            ...
            attn_sparsity, attn_total, attn_nonzero = calculate_sparsity(model, "encoder_attn")
            ...
            print(f"Encoder attention sparsity: {attn_sparsity:.2f}%")
            print(f"Total parameters: {total_params:,}")
            print(f"Non-zero parameters: {non_zero_params:,}")
            print(
                f"Encoder attention parameters: {attn_total:,} ({attn_total/total_params*100:.2f}% of model)"
            )
...
def create_plots(results, metric_names, plot_dir, encoder_attention_only=True):
    """
    ...
    Args:
        ...
        encoder_attention_only: Whether this is encoder attention-only pruning (True) or whole model pruning (False)
    """
    print("\nGenerating plots...")
    ...
    for model_name, model_results in results.items():
        if "baseline" in model_name:
            ...
            try:
                if encoder_attention_only:
                    # For encoder_attn-only models (format: "l1_encoder_attn_XX_clean" or "l1_encoder_attn_XX_other")
    ...
    pruning_type = "Encoder Self-Attention Only" if encoder_attention_only else "Whole Model"
    ...
    for percent in pruning_percentages:
        ...
        if percent == 0:
            ...
            if encoder_attention_only:
                model_name = f"l1_encoder_attn_{percent}_clean"
                if model_name not in results:
                    model_name = f"l1_encoder_attn_{percent}_other"
            else:
...
def main():
    ...
    # Define the pruning percentages to test (0% to 50% in 10% increments for encoder-attention-only)
    ...
    for percent in pruning_percentages:
        if percent > 0:  # Skip 0% as it's already in baseline
            model_configs[f"l1_encoder_attn_{percent}"] = {
                "pruning_amount": percent / 100,  # Convert percentage to fraction
                "encoder_attention_only": True,  # Only prune attention components in the encoder
            }
    ...
    for model_name, config in model_configs.items():
        ...
        try:
            ...
            model = load_whisper_model(
                ...
                make_permanent=True,
                encoder_attention_only=config.get("encoder_attention_only", False),
            )
                ...
                try:
                    ...
                    if isinstance(scores, dict) and "error" not in scores:
                        ...
                        results[f"{model_name}_{split}"] = {
                            ...
                            "encoder_attention_only": config.get(
                                "encoder_attention_only", False
                            ),  # Add encoder_attention_only flag
    ...
    create_plots(
        results=results, metric_names=safe_metrics, plot_dir=PLOTS_DIR, encoder_attention_only=True
    )

    # Print summary
    print("\n" + "=" * 60)
    print("L1 UNSTRUCTURED ENCODER SELF-ATTENTION ONLY PRUNING EXPERIMENT SUMMARY")
    print("=" * 60)
    ...
    print("\nResults for different encoder attention pruning percentages:")
    for percent in pruning_percentages:
        if percent == 0:
            continue

        model_key = f"l1_encoder_attn_{percent}_clean"
        if model_key in results:
            ...
            print(f"\n{percent}% encoder self-attention pruning:")
            ...
            if "attn_sparsity" in result:
                print(f"  Encoder Attention Sparsity: {result['attn_sparsity']:.2f}%")
            ...
            if "attn_parameters" in result:
                print(
                    f"  Encoder Attention Parameters: {result['attn_parameters']:,} ({result['attn_parameters']/result['total_parameters']*100:.2f}% of model)"
                )
    ...
    print(
        "\nNote: This experiment focused on pruning only the self-attention components in the encoder of the Whisper model"
    )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("WHISPER ENCODER SELF-ATTENTION ONLY L1 UNSTRUCTURED PRUNING EXPERIMENT")
    print("=" * 80)
    print("This experiment applies L1 unstructured pruning ONLY to the self-attention components")
    print("in the encoder part of the Whisper model.")
